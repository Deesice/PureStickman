...
Shader "Skybox Gradient"
{
	Properties
	{
		...
		_Vignette("Vignette", Color) = (0,0,0,0)
		_mult("mult", Float) = 1
		_pwer("pwer", Float) = 1
		_radius("radius", Float) = 1
		_intensity("intensity", Float) = 1
		_vpwer("vpwer", Float) = 1
	...
	LOD 100
		...
		Pass
		{
			...
			uniform float4 _Top;
			uniform float4 _Vignette;
			uniform float _mult;
			uniform float _pwer;
			uniform float _radius;
			uniform float _intensity;
			uniform float _vpwer;
			...
			fixed4 frag (v2f i ) : SV_Target
			{
				...
				float2 pos = ase_screenPosNorm.xy;
				pos.x *= _textureScale * _ScreenParams.x / _ScreenParams.y;
				pos.y *= _textureScale;

				lerpResult3 += tex2D(_Texture, pos) * _textureIntensity;
				
				ase_screenPosNorm.x -= 0.5;
				ase_screenPosNorm.x *= 2;
				ase_screenPosNorm.y -= 0.5;
				ase_screenPosNorm.y *= 2;
				finalColor = lerp(lerpResult3,
					lerp(lerpResult3, _Vignette, pow(length(ase_screenPosNorm.xy), _vpwer) / _radius),
					_intensity);
				return finalColor;
			}
			ENDCG
